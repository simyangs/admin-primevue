...
const menuList: AppMenuItem[] = [
  { key: 'M000', label: '대시보드', icon: 'pi pi-home', to: '/' },
  {
    key: 'C001',
    label: '관리1',
...
const addMenuItem = (menu: AppMenuItem) => {
  const menuItem: AppMenuItem = { key: menu.key, label: menu.label, icon: menu.icon };
  if (menu.to) {
    menuItem.to = menu.to;
  } else if (menu.items) {
...
<template>
  <aside :class="['sidebar', isSidebarCollapsed ? 'collapsed' : 'expanded']">
